...
function startQuiz() {
    ...
    }).then(onSuccess, onFail).then(onInit);
...
function onInit(json) {
    ...
    form.id = DATA._id;
    form.method = 'POST';
    form.action = "https://wpr-quiz-api.herokuapp.com/attempts/" + form.id + "/submit";
    form.target = '_self';
...
function submitQuiz() {
    ...
    fetch("https://wpr-quiz-api.herokuapp.com/attempts/" + form.id + "/submit", {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json'
        },
        body: JSON.stringify(user_answers)
    }).then(onSuccess, onFail).then(onSubmit);
